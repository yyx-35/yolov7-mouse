...
import test_Ncla as test    # import test.py to get mAP after each epoch
from models.experimental import attempt_load
from models.yolo_Ncla import Model
...
from utils.loss_Ncla import ComputeLoss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        run_id = torch.load(weights,map_location=torch.device('cpu')).get('wandb_id') if weights.endswith('.pt') and os.path.isfile(weights) else None
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            kpt_label=kpt_label,kpt_num=kpt_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='yolov7-w6-pose.pt', help='initial weights path')
    ...
    parser.add_argument('--sync-bn', default=True, help='use SyncBatchNorm, only available in DDP mode')
