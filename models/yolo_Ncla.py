...
class IKeypoint(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference    True
                if self.grid[i].shape[2:4] != x[i].shape[2:4]:
                    self.grid[i] = self._make_grid(nx, ny).to(x[i].device)
                kpt_grid_x = self.grid[i][..., 0:1]
                kpt_grid_y = self.grid[i][..., 1:2]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, ConvFocus, CrossConv, BottleneckCSP,
                 C3, C3TR, BottleneckCSPF, BottleneckCSP2, SPPCSP, SPPCSPC,Conv_CBAM,CoordConv,DSConv2D,Conv_SA,DySnakeConv,EVCBlock
                ]:
            c1, c2 = ch[f], args[0]
            ...
            if m in [BottleneckCSP, C3, C3TR, BottleneckCSPF, BottleneckCSP2, SPPCSP, SPPCSPC]:
                ...
                n = 1
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, DWConv, MixConv2d, Focus, ConvFocus, CrossConv, BottleneckCSP, C3, C3TR,Conv_CBAM,CoordConv,DSConv2D,Conv_SA,DySnakeConv,gnconv]:
                if 'act' in d.keys():
                    args_dict = {"act" : d['act']}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='E:/good/cfg/yolov7-w6-pose.yaml', help='model.yaml')
