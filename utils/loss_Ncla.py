...
class WingLoss(nn.Module):
    def __init__(self, w=10, e=2):
        ...
        self.w = w
        self.e = e
        #self.C = self.w - torch.tensor(9 * np.log(1 + (self.w + 1) / 2) + np.log(self.w), dtype=torch.float32)
        #self.CC = torch.tensor(10.1419, dtype=torch.float32)
    ...
    def forward(self, x, t, sigma=1):
        ...
        #y = flag * (9 * torch.log(1 + (abs_diff + 1)/ 2) + torch.log(abs_diff)) + (1-flag) * (abs_diff - self.C)
        return y.sum()
