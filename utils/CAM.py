...
def get_params():
    params = {
        'weight': 'E:/good/runs/train/dayandnight/weights/best.pt', # 这选择想要热力可视化的模型权重路径
        'cfg': './cfg/yolov7-w6-pose_yuan.yaml', # 这里选择与训练上面模型权重相对应的.yaml文件路径
...
if __name__ == '__main__':
    ...
    model('datasets/defect/images/test/frame_adjusted_5.png', './result') # 第一个参数是图片的路径，第二个参数是保存路径，比如是result的话，其会创建一个名字为result的文件夹，如果result文件夹不为空，其会先清空文件夹。
