...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, kpt_label=False,kpt_num=17):
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = torch.tensor([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89][:self.kpt_num], device=device) / 10.0
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            # gain = torch.ones(41, device=targets.device).long()  # normalized to gridspace gain
            gain = torch.ones(self.kpt_num*2 +7 , device=targets.device).long()  # normalized to gridspace gain
        else:
